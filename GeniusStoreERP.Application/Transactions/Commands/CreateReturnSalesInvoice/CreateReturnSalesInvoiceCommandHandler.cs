    ...
    public async Task<int> Handle(CreateReturnSalesInvoiceCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var partnerTransaction = new PartnerTransaction
            {
                ...
                Credit = invoice.FinalAmount, // دائن بقيمة المرتجع لأنها مبيعات تنقص المديونية
                Remarks = $"مرتجع مبيعات رقم {invoice.InvoiceNumber} "
            };
            ...
            foreach (var item in invoice.InvoiceItems)
            {
                ...
                if (product != null)
                {
                    ...
                    var stockMovement = new StockTransaction
                    {
                        ...
                        Remarks = $"مرتجع مبيعات رقم {invoice.InvoiceNumber} - العميل: {partner?.Name ?? "عميل عام"}"
                    };
