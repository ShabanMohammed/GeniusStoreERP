...
public class UpdatePartnerCommandHandler : IRequestHandler<UpdatePartnerCommand>
{
    ...
    public async Task Handle(UpdatePartnerCommand request, CancellationToken cancellationToken)
    {
        ...
        var ExiteNamePartner = await _context.Partners
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(p => p.Name == request.Name.Sanitize() && p.Id != request.Id, cancellationToken);
        if (ExiteNamePartner != null)
        {
            if (ExiteNamePartner.IsDeleted)
                throw new EntityDeletedException(ExiteNamePartner);
            throw new EntityConflictException(ExiteNamePartner);
